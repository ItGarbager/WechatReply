...
from wechat.monitor.message import handle_event
...
class WX:
    def __init__(self):
        self.wx = WechatPCAPI(on_message=self._on_message, log=logger)
        ...
        try:
            ...
            if res:
                ...
                if send_or_recv and data.get('time') >= START_TIME:
                    ...
                    # 异步启动当前注册的事件，插件目录 wechat/plugins/
                    asyncio.run(handle_event(Message(data, chat_type, group, user, msg, self)))
...
class Message:
    def __init__(self, data, chat_type, group, user, msg, wx: WX):
        self.data = data
